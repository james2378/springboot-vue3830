		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/position_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_position_information = !show_list_position_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_position_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--职位信息推荐列表组件 -->
		                <list_position_information v-if="show_list_position_information" :list="list_position_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_position_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_position_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_position_information" :key="i" @click="to_details('position_information',o,'position_information_id')">
		                            <td v-for="(oj,n) in fields_position_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	import list_position_information from "@/components/diy/list_position_information.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_position_information: true,
				list_position_information: [],
				vm_arr_position_information: [
					...
					"number_of_recruits",
					"recruitment_company",
					"resume_delivery_address",
					"job_requirements",
					"salary",
					"details",
				],
				fields_position_information: [
					{
						key: "job_title",
						label: "职位名称",
						type: "文本"
		...
		methods: {
			...
			get_position_information() {
				let url = "~/api/position_information/get_list?";
				url = "~/api/position_information/get_hits_list?";
				this.$get(url, {
					...
					if (json.result) {
						this.list_position_information = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_position_information() {
		        var list = this.list_position_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_position_information.map((o) => {
		                // 第二个中括号是对象的属性
